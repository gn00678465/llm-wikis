...
as `v0.1.0-beta.2` is never picked up by an unpinned run. If no stable
...
download error. Set `LLM_WIKIS_VERSION` (an exact tag, e.g. `v0.1.0-beta.2`)
before running either script to install a specific version instead — this
pins, upgrades, or downgrades in place:

```sh
curl -fsSL https://raw.githubusercontent.com/gn00678465/llm-wikis/refs/heads/main/install.sh | LLM_WIKIS_VERSION=v0.1.0-beta.2 sh
```

(The variable must be scoped to `sh` — the process that actually reads it — not to `curl`. `LLM_WIKIS_VERSION=v0.1.0-beta.2 curl ... | sh` sets the variable only for `curl`'s environment and never reaches the piped `sh`.)

```powershell
$env:LLM_WIKIS_VERSION = "v0.1.0-beta.2"
irm https://raw.githubusercontent.com/gn00678465/llm-wikis/refs/heads/main/install.ps1 | iex
```

...
- **No `llm-wikis` command has a code path that writes to a knowledge base** (§7.2 item 17) — the harness's other mechanical layers (§7.2 items 5-8, §12) additionally block or detect a mutation attempted through the invoked provider itself, with the honestly-stated residuals recorded there: a narrowed-not-closed check-to-spawn race around the Claude wiki-settings check (§10.2), and an admin-managed/enterprise-policy hook that no flag or check in this project can reach.

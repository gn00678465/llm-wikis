...
The wiki's own project/local settings **are** loaded (required for skill discovery), and the argv flags above are **not** the load-bearing trust boundary by themselves (corrected again by R-29, §23): a live test found that `apiKeyHelper` — a key that is neither `hooks` nor tool-shaped — executed a command even with `--settings {"disableAllHooks":true}` present. R-29 therefore adds a **preflight deny check** (§12, §15) that statically inspects `project_root/.claude/settings.json`/`settings.local.json` and refuses the wiki outright, before this argv is ever built, unless every top-level key is in a small allowlist (`enabledPlugins`, `permissions.{allow,deny,defaultMode}`). Everything else — `apiKeyHelper`, `awsCredentialExport`, `awsAuthRefresh`, `gcpAuthRefresh`, `otelHeadersHelper`, `statusLine` (all run a configured command), `permissions.additionalDirectories` (widens read reach beyond `project_root`), `env` (can redirect API traffic), `hooks`, and any key not yet enumerated — fails the wiki closed. This check runs at both `doctor` and `query` time, closing the TOCTOU window between a passing `doctor` run and a later `query` invocation.
...
- Claude wiki-side settings surface denied by allowlist at both `doctor` and `query` time (§10.2/§15 R-29 — `project_root/.claude/settings.json`/`settings.local.json` are loaded regardless of trust, and several keys beyond `hooks` execute a command or widen reach on their own, e.g. `apiKeyHelper`, live-confirmed to execute even with hooks disabled; only `enabledPlugins` and `permissions.{allow,deny,defaultMode}` are admitted, everything else fails the wiki closed as `ENTRYPOINT_INVALID`), plus `--settings {"disableAllHooks":true}` (R-27/R-28) as defense in depth for the admitted surface — project/local settings are still loaded, since skill discovery depends on them, so reach is bounded by these layers rather than by non-loading; the honest residual gap is that no flag or check disables an admin-managed/enterprise-policy hook; Codex's equivalent project-level surface is trust-gated by its own design and was not extended with a matching check (investigated, §10.2);
...
- Claude only: the wiki's own `project_root/.claude/settings.json`/`settings.local.json` declare no key outside the closed allowlist (`enabledPlugins`, `permissions.{allow,deny,defaultMode}`) — §10.2/§12 R-29, under check name `entrypoint`; re-run identically at query time, not only at doctor time;

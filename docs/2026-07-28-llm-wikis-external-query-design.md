...
**How these are guaranteed — and what "guaranteed" means here.** Items 1, 2, 3, 4, and 9 are carried by the envelope's `constraints` and `required_result`, and item 9 is additionally enforced mechanically by the provider's output schema (§10.2, §10.3) and validated by the wrapper (§7.3, §11). Items 5 through 8 describe *model behavior* the prompt instructs — the harness cannot mechanically prevent a model from attempting to violate them or from mentioning that it would (that would require controlling the model's own reasoning, which is out of scope) — so "enforced mechanically" means something narrower and more precise: an attempt cannot *succeed*, and if one nonetheless lands, it is *detected*. Items 6-8 (never regenerate the index, never write, never attempt mutation through any tool) are backed by genuine write-prevention: the tool set exposes no Write, Edit, Bash, or web tool at all, so under Claude there is no mechanism such an attempt could act through; Codex runs under a read-only sandbox that blocks any write its own tools might attempt, regardless of what the model tries. On top of that prevention layer, §12's before/after content snapshot detects a mutation anywhere in the `content_root` tree it actually covers — every directory and file beneath `content_root` **except** the two excluded `.claude/`/`.agents/` top-level directories (§12); a mutation confined entirely to one of those two excluded trees (e.g. from an admin-managed hook, or the residual check-to-spawn race stated in §10.2) would not be caught by this layer. Item 5 (omit answer-saving offers) has **no mechanical backstop at all** — unlike 6-8, there is nothing to prevent or detect, because offering to save is text in the answer, not a tool call; it is prompt-instructed behavior only, same as item 5's own residual-risk discussion below already implies.
...
- **No `llm-wikis` command has a code path that writes to a knowledge base** (§4 Design Decision 17) — the harness's other mechanical layers (tool absence and the Codex sandbox prevent a write attempted through the invoked provider from succeeding; §12's snapshot detects one that nonetheless lands, per §7.2's "How these are guaranteed") additionally cover the provider itself, with the honestly-stated residuals recorded there: a narrowed-not-closed check-to-spawn race around the Claude wiki-settings check (§10.2), and an admin-managed/enterprise-policy hook that no flag or check in this project can reach.
...
- `query` exposes no tool through which the provider could regenerate an index, update a log, or modify wiki content — Claude has no Write/Edit/Bash/web tool at all, and Codex's read-only sandbox blocks any write its own tools might attempt (§7.2). Saving an answer or claiming to have mutated something is prompt-instructed model behavior with no mechanical prevention of the attempt itself (§7.2 item 5) — only of the write succeeding.
